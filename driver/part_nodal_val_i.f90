program gedatsu_nodal_val_i_partitioner
  ...
  if(is_get)then
    write(*,"(a)")"usage:"
    write(*,"(a)") &
    & "./gedatsu_nodal_val_i_partitioner {options} -n {number of domains}"
    write(*,"(a)")""
  ...
  do i = 1, n_domain
    ...
    call monolis_alloc_I_2d(val_local, n_dof, graph%n_vertex)
    ...
    call monolis_output_distval_i(foname_full, label, graph%n_vertex, n_dof, val_local)
