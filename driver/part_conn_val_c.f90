program gedatsu_partitioner_connectivity_val_r
  ...
  call gedatsu_std_log_string("gedatsu_connectivity_val_i_partitioner")
  ...
  if(is_get)then
    write(*,"(a)")"usage:"
    write(*,"(a)") &
    & "./gedatsu_connectivity_val_i_partitioner {options} -n {number of domains}"
    write(*,"(a)")"- output filename: {input filename}.{domain_id}"
    write(*,"(a)")""
    write(*,"(a)")"-n {number of domains}: (default) 1"
    write(*,"(a)")"-i {input filename}: (default) conn_val.dat"
    write(*,"(a)")"-id {input id filename}: (default) connectivity.dat.id"
...
end program gedatsu_partitioner_connectivity_val_r
